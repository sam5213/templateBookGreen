document.addEventListener("DOMContentLoaded", () => {
  ...
  async function sendDataToServer() {
    ...
    const data = {
      user: {
        id: user?.id || "test_user_id", // ID пользователя Telegram (или тестовое значение)
    ...
    try {
      ...
      const response = await fetch("https://your-server.com/api/book", {
        method: "POST",
        headers: {
  ...
  function handleTimeSelect(time) {
    ...
    const selectedSlot = document.querySelector(`.time-slot:contains("${time}")`);
    if (selectedSlot) {
